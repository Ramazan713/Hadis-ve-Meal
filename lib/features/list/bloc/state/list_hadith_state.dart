...
class ListHadithState extends IListCountState{
  const ListHadithState({
    DataStatus status = DataStatus.initial,
    List<IListView> listItems=const []
  }):super(status: status,listItems: listItems);

  ListHadithState copyWith({DataStatus? status, List<IListView>? listItems}) {
    return ListHadithState(
